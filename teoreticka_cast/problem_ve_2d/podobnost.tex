...
\label{sec:podobnost}
V \myref{sekci}{sec:algoritmus} vám představím algoritmus na hledání trojúhelníku s minimálním obvodem. Velmi důležitou částí algoritmu je kontrola, zda body neleží na přímce, a jestli skutečně tvoří trojúhelník. K zkontrolování máme následující tvrzení.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    b_y-a_y & = m(b_x-a_x)
  \end{align*}
  A tím pádem $m = (a_y-c_y)/(a_x-c_x)$ a zároveň $ m=(b_y-a_y)/(b_x-a_x)$. Dále můžeme sestavit rovnici $(a_y-c_y)/(a_x-c_x) = (b_y-a_y)/(b_x-a_x)$. Nakonec můžeme rovnici upravit do tvaru $(a_y-c_y)(b_x-a_x) = (b_y-a_y)(a_x-c_x)$. Tímto je \myref{tvrzení}{tvrzeni:podobnost_trojuhelniku} dokázáno.
\end{proof}