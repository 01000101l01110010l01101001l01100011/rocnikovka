...
\begin{definition}[Podobnost trojúhelníků a body na jedné přímce]
    \label{definice:podobnost_trojuhelniku}
    Jsou dány 3 body: $a = (a_x, a_y)$, $b = (b_x, b_y)$, $c = (c_x, c_y)$. Tyto body jsou na přímce, pokud platí, že trojúhelníky $(a_x, a_y), (b_x, a_y), (b_x, b_y)$ a $(a_x, a_y), (c_x, a_y), (c_x, c_y)$ mají stejný poměr stran, neboli platí: $\frac{\textcolor{myred}{c_y-a_y}}{\textcolor{mygreen}{c_x-a_x}} = \frac{\textcolor{myorange}{b_y-a_y}}{\textcolor{myblue}{b_x-a_x}}$.
    ...
    \begin{tikzpicture}
        ...
        \draw[myblue, line width=1.5pt] (A) to node[midway, below] {$1$} (D);
        \draw[mygreen, line width=1.5pt] (D) to node[midway, below] {$1$} (E);
        \draw[myred, line width=1.5pt] (C) to node[midway, right] {$6$} (E);
        \draw[myorange, line width=1.5pt] (B) to node[midway, right] {$3$} (D);
        \fill (A) circle[radius=2pt];
        \fill (B) circle[radius=2pt];
        \fill (C) circle[radius=2pt];
