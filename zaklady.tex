...
\begin{tvrzeni}[Trojúhelníková nerovnost]
...
\begin{definition}[Obvod polytopu]
...
\begin{definition}[Landauova notace]
  \label{definice:bigonotation}
  Vizte například \autocite{Bae2019}. Velmi krátce řečeno: notace $\mathcal{O}(\dots)$ představuje počet kroků, které algoritmus musí udělat až na násobení konstantou nebo přičtení konstanty. O algoritmu můžeme říct, že má časovou složitost například $\mathcal{O}(n^2)$, kde $n$ je délka vstupu.
  ...
  \begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Grafy funkcí, které jsou častými případy časové náročnosti algoritmů.} \label{obr:big_o}
  \end{figure}
\end{definition}